...
export default function OrderTracker() {
  ...
  const fetchUserData = async () => {
    try {
      const response = await axios.get(import.meta.env.VITE_SERVER_USER_LINK +`/users/api/user/${id}`);
      if(response){
...
const fetchRemainingDays = async (invoice) => {
  try {
    ...
    const response = await axios.get(import.meta.env.VITE_SERVER_INVOICE_LINK + `/invoices/payment/invoices/${invoice._id}/remaining-days`);
    if (response) {
