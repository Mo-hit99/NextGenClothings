...
export default function Products() {
  ...
  // Filter data based on category, brand, and search query
  const filteredData = data.filter((product) => {
    return (
      // If categoryItem is provided, only include products in that category
      (!categoryItem || product.category === categoryItem) &&
      // Check if the product category is included in the selected categories or if no category filter is applied
      (category.length === 0 || category.includes(product.category)) &&
      // Check if the product brand is included in the selected brands or if no brand filter is applied
      (brand.length === 0 || brand.includes(product.brand)) &&
      // Check if the product brand matches the search query (if provided)
      (!queryFromUrl || product.brand.toLowerCase().includes(queryFromUrl.toLowerCase()))
    );
  });
  ...
  //         }/productData/?search=${encodeURIComponent(query)}`
  //       );
  //       if (response) {
  ...
  async function filterSubmit(e) {
    ...
    try {
      const response = await axios.get(
        ...
        }/products/productData?category=${category}&brand=${brand}`
      );
          ...
          <div className="filter-product-containe2r">
            <form className="filter-form-section2" onSubmit={filterSubmit}>
              <p className="filter-para2">
                ...
                <span className="filter-para-result2">
                  Results({data.length})
                </span>
              </p>
